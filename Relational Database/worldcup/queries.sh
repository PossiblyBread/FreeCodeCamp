...
echo "$($PSQL "SELECT DISTINCT teams_winner.name 
              FROM games
              INNER JOIN teams teams_winner ON games.winner_id = teams_winner.team_id
              WHERE games.year = 2014 AND games.round = 'Eighth-Final'
              UNION
              SELECT DISTINCT teams_opponent.name
              FROM games
              INNER JOIN teams teams_opponent ON games.opponent_id = teams_opponent.team_id
              WHERE games.year = 2014 AND games.round = 'Eighth-Final'
              ORDER BY name;")"
